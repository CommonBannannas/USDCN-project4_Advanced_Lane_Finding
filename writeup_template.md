...
* Advanced_Lane_Finding.ipynb: Notebook containing all the project and the process.
* utils.py: Utilities library for the project, a compilation with alterations of functions from: [Jessica Yung's github](https://github.com/jessicayung/self-driving-car-nd/blob/master/p4-advanced-lane-lines/helperfunctions.py)
* pipeline.py: The pipeline for processing the frames.
* project_output_colour.mp4: Processed video with the lane detection.
...
To identify the lane pixels:

* The function `` hist_pixels `` divides the image into horizontal slices, then counts all the pixels with respect to x. Then it smoothens the histogram using scipy's function: `` scipy.signal.medfilt `` . `` scipy.signal.find_peaks_cwt `` finds the left and right peaks on the smoothened histogram. 
